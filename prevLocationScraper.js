...
// var instagramTag = "aaa";
// var clickMaxNum = "1000";
var linkArray=casper.cli.get(2);
...
function getContent() {
 ...
 // var fs = require('fs');
 // elementsHrefs=fs.open(linkArray,'r')
 var jsonDetails=require(linkArray);
 var elementsHrefs=[];
 for (var i=0;i<jsonDetails.length;i++){
    casper.echo('page url: '+i+" is "+JSON.stringify(jsonDetails[i]["page_url"]))
    elementsHrefs.push(jsonDetails[i]["page_url"])
 }

 require('utils').dump(elementsHrefs);
 casper.echo(elementsHrefs);
 elemNum = elementsHrefs.length;
 casper.echo("Openlinks: "+elemNum, 'INFO');
    // casper.then(function() {
...
casper.start(baseUrl, getContent);
